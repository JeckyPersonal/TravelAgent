...
namespace Invoice.Service
{
    public class BankDetailService : IBankDetailService
    {
        ...
        public async Task<BankDetail> Add(BankDetail entity)
        {
            ...
            this._assertService.AssertDuplicationEntity(x => x.AccountNumber.Equals(entity.AccountNumber), x => !x.Id.Equals(entity.Id), "BankdDetail");

            return await this._invoiceRepository.Add(entity);
        }

        public async Task<BankDetail> Delete(int id)
        {
            BankDetail bankById = await this.Get(id);

            return await this._invoiceRepository.Delete(bankById);
        }
        ...
        public async Task<BankDetail> Get(int id)
        {
            ...
            return await this._assertService.AssertEntityExist(x => x.Id.Equals(id), nameof(BankDetail));
        ...
        public async Task<BankDetail> Update(BankDetail entity)
        {
            ...
            BankDetail existingDetail = await this._assertService.AssertEntityExist(x => x.Id == entity.Id, nameof(BankDetail));
